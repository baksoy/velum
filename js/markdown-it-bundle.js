...
`;
  ...
  function abbr_plugin(md2) {
    const escapeRE3 = md2.utils.escapeRE;
    const arrayReplaceAt2 = md2.utils.arrayReplaceAt;
    const OTHER_CHARS = " \r\n$+<=>^`|~";
    const UNICODE_PUNCT_RE = md2.utils.lib.ucmicro.P.source;
    const UNICODE_SPACE_RE = md2.utils.lib.ucmicro.Z.source;
    ...
    function abbr_replace(state) {
      ...
      const regText = "(^|" + UNICODE_PUNCT_RE + "|" + UNICODE_SPACE_RE + "|[" + OTHER_CHARS.split("").map(escapeRE3).join("") + "])(" + Object.keys(state.env.abbreviations).map(function(x) {
        return x.substr(1);
      }).sort(function(a, b) {
        return b.length - a.length;
      }).map(escapeRE3).join("|") + ")($|" + UNICODE_PUNCT_RE + "|" + UNICODE_SPACE_RE + "|[" + OTHER_CHARS.split("").map(escapeRE3).join("") + "])";
  ...
  var md = new lib_default({
    ...
    typographer: true,
    breaks: false
  }).use(footnote_plugin).use(deflist_plugin).use(abbr_plugin).use(import_markdown_it_task_lists.default, { enabled: false });
